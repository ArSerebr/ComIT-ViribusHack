...
> При деплое — заменить на актуальный хост. Nginx проксирует все запросы с порта `8083` на FastAPI бэкенд.

---

## Идентификация пользователя (uid)

Каждый пользователь идентифицируется по `uid` — произвольной строке, которую **генерирует и хранит фронтенд**.
...
2. GET  /api/task/{id}    → поллить до status == "READY"
3. Показать result из ответа
```

Параллельно можно поллить `/api/status` для отображения прогресс-бара.
